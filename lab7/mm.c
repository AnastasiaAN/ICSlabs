/*
 * mm.c 
 * Data structure: explict list and binary tree
 * Use explicit lists to maintain smaller free block lists
 * Use a binary search tree to maintain larger free block lists
 * All linked lists have only one same size of free blocks
 * Thanks ^^
 * 
 * By Wang Ziyu 5140309052 2017.03.09
 */
#include <stdio.h>
#include <stdlib.h>
#include <assert.h>
...
/*********************************************************
 * NOTE TO STUDENTS: Before you do anything , please
...
extern int mm_init(void);//extern functions
extern void *mm_malloc (size_t size);
extern void mm_free (void *ptr);

static char *mem_heap;//Private global variables
static char *mem_brk;
...
#define PUT_LEFT_CHILD(bp,val) (PUT(LEFT(bp),(long)val))
#define PUT_RIGHT_CHILD(bp,val) (PUT(RIGHT(bp),(long)val))
#define GET_LEFT_CHILD(bp) (GET(LEFT(bp)))
#define GET_RIGHT_CHILD(bp) (GET(RIGHT(bp)))

//get or set the head or foot of bp
#define PUT_HEAD(bp,val) (PUT(HEAD(bp),(long)val))
#define PUT_FOOT(bp,val) (PUT(FOOT(bp),(long)val))
...
#define PUT_PAR(bp,val) (PUT(PRNT(bp),(long)val))
#define PUT_BROS(bp,val) (PUT(BROS(bp),(long)val))
...
static void check(void *bp)
{
	...
	printf("0x%x[%d/%d] ~ 0x%x[%d/%d]\n",HEAD(bp),GET_HEAD_SIZE(bp),GET_HEAD_ALLOC(bp),FOOT(bp),GET_HEAD_SIZE(bp),GET_HEAD_ALLOC(bp));
	printf("0x%x[%d/%d] ~ 0x%x[%d/%d]\n\n",HEAD(NEXT_BLKP(bp)),GET_HEAD_SIZE(NEXT_BLKP(bp)),GET_HEAD_ALLOC(NEXT_BLKP(bp))\
